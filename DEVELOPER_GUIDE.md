...
- 生成页经网关代理访问 ComfyUI(`/api/generate/view`,地址取自 `COMFYUI_BASE_URL`
  配置,未配置时生成面板不可用)——前端不直连 ComfyUI 端口
...
- **声明受众**:每份文档开头写明面向谁(使用者 / 部署者 / 解析器贡献者),以及前置阅读(若有)
- **引用只向下**:概览 → 细节方向引用,不形成循环;链接是"查阅边"(用到再点开),
  不制造"理解链"(不先读 A 就读不懂 B)
- **核心结论自包含**:把某份文档的全部外链想象成失效,其核心结论仍应成立;
  依赖外部上下文才能成立的内容,内联进来
- **面向下游的文字不夹带维护者机制**:内部流程(验证流水线、构建输入声明等)
  不写入公开文档;下游只需要"做什么、产物语义是什么"
- **入口路由集中在 README**:新文档加入后更新 README 的文档路由表,不在其他文档里互相扩散入口
