...
function statsNormalizeTagTerm(value) {
  return String(value ?? "")
    ...
    // 匹配口径与主页一致:内容须完整相等,只容忍分隔符差异(见 app.js 注释)
...
const STATS_TAG_TONE_COUNT = 12;

// 色号:身份哈希定基调,同一批标注内不重复用色号(与主页 pickTagTone 同源)。
function statsPickTagTone(identity, usedTones) {
  const seed = statsTagHash(identity) % STATS_TAG_TONE_COUNT;
  if (!usedTones.has(seed)) return seed;
  let best = seed;
  let bestGap = -1;
  for (let tone = 0; tone < STATS_TAG_TONE_COUNT; tone += 1) {
    if (usedTones.has(tone)) continue;
    let gap = Infinity;
    for (const taken of usedTones) {
      const raw = Math.abs(tone - taken);
      gap = Math.min(gap, Math.min(raw, STATS_TAG_TONE_COUNT - raw));
    }
    if (gap > bestGap) {
      bestGap = gap;
      best = tone;
...
// 标注目录 + 词条索引(与主页 buildTagCatalog/tagIndexFromCatalog 同款):
// 命中判定按"整条标注的全部词条齐备",不能只看单个词条。
function statsBuildTagCatalog(items) {
  const entries = [];
  ...
  // 稳定顺序:配色只取决于标注集合,与接口返回顺序无关(与主页同款)
  ...
  return entries.map(({ item, terms, identity }) => {
    const tone = statsPickTagTone(identity, usedTones);
    usedTones.add(tone);
    return {
...
function statsTagIndexFromCatalog(catalog) {
  const index = new Map();
...
function statsBuildTagIndex(items) {
  return statsTagIndexFromCatalog(statsBuildTagCatalog(items));
}

// 生效标注集合:声明的全部词条都出现在给定文本里才生效(与主页 tagIndexForPrompts
// 同款口径);生效后它的所有词条统一染色。
function statsTagIndexForTexts(texts) {
  const catalog = statsState.tagCatalog;
  if (!Array.isArray(catalog) || !catalog.length) {
    return statsState.tagIndex instanceof Map ? statsState.tagIndex : new Map();
  }
  const present = new Set();
  for (const text of Array.isArray(texts) ? texts : [texts]) {
    for (const segment of String(text ?? "").replace(/\r\n/g, "\n").split(/[\n,]+/)) {
      const term = statsNormalizeTagTerm(segment);
...
async function loadStatsTagCatalog() {
  try {
    ...
    statsState.tagCatalog = statsBuildTagCatalog(payload?.items || []);
    statsState.tagIndex = statsTagIndexFromCatalog(statsState.tagCatalog);
  } catch {
    statsState.tagCatalog = [];
    statsState.tagIndex = new Map();
  }
}

function statsTagForSegment(value, index = null) {
  const lookup = index instanceof Map ? index : statsState.tagIndex;
  return lookup instanceof Map ? lookup.get(statsNormalizeTagTerm(value)) : undefined;
}

// prompt 层文本切段标注:按逗号/换行切分;命中**全部词条齐备**的标注的段渲染
// tag-highlight 染色按钮,其余段包 danbooru-seg(悬停查关联)。切段与转义规则
// 与主页 highlightPromptText 一致;生效标注集合按本单元格文本判定。
function statsHighlightPrompt(value) {
  const index = statsTagIndexForTexts([value]);
  return String(value ?? "")
    .replace(/\r\n/g, "\n")
    .split(/(,|\n)/)
    .map((segment) => {
      ...
      const tag = statsTagForSegment(core, index);
      if (tag) {
