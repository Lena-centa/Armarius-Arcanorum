const state = {
  ...
  tagIndex: new Map(), // 词条 → 标注(查表用)
  tagCatalog: [], // 标注目录:整条标注 + 全部词条("全部词条齐备"判定用)
  ...
  favoriteRecords: new Map(), // 收藏视图参数摘要:batch_key → 批次详情(卡片一次性补全)
...
async function loadTagCatalog() {
  try {
    ...
    // 目录(整条标注 + 全部词条)供"全部词条齐备才算命中"判定;索引供词条级查表
    state.tagCatalog = buildTagCatalog(payload?.items || []);
    state.tagIndex = tagIndexFromCatalog(state.tagCatalog);
    ...
    .map((prompt, index) => {
      ...
      const sourceLayers = Array.isArray(prompt.layers) && prompt.layers.length
        ...
        : prompt.text
          ...
          : [];
      const isSingleLayer = sourceLayers.length <= 1;
      const layers = sourceLayers
        .map((layer, lIdx) => {
          const lines = layer.lines && layer.lines.length ? layer.lines : [layer.text || ""];
          const metaHtml = isSingleLayer
            ? ""
            : `<div class="prompt-layer-meta"><span class="prompt-layer-label">[L${Number(layer.layer_index || lIdx) + 1}]</span><button class="prompt-layer-copy" type="button" data-copy-layer aria-label="复制当前 Layer">复制</button><button class="prompt-layer-toggle" type="button" data-toggle-layer aria-label="折叠/展开当前 Layer" title="折叠/展开">−</button></div>`;
          return `<div class="prompt-layer">${metaHtml}<div class="prompt-pre">${highlightPromptText(lines.filter(Boolean).join("\n"), (options && options.tagIndex) || null)}</div></div>`;
        })
        .join("");
      const branch = options && options.branch ? ` data-prompt-branch="${escapeHtml(options.branch)}"` : "";
      return `<div class="prompt-entry${defaultCollapsed ? " is-collapsed" : ""}"${branch}><div class="prompt-entry-header"><div class="prompt-entry-label">${escapeHtml(label)}</div><div class="prompt-entry-actions"><button class="prompt-layer-copy" type="button" data-copy-layer aria-label="复制当前提示词">复制</button><button class="prompt-entry-toggle" type="button" data-toggle-entry aria-label="折叠/展开整体" title="折叠/展开整体">${defaultCollapsed ? "+" : "−"}</button></div></div><div class="prompt-entry-body">${layers}</div></div>`;
    })
    .join("");
}
...
function normalizeTagTerm(value) {
  return String(value ?? "")
    ...
    // 匹配口径:正文片段与词条的**内容必须完整相等**,只容忍分隔符差异
    // (空格/下划线互换、连续空白折叠、大小写)。因此不剥权重/括号包装——
    // "{{{best_quality" 与词条 "best_quality" 内容不等,不该算命中
    // (NAI 加权写法要命中就把整段原文写成词条)。标注词条与 prompt 片段
    // 两侧共用本函数,故口径对称。
...
const TAG_TONE_COUNT = 12;

// 色号:身份哈希定基调,但同一批标注内不重复用色号——一张图挂多条标注时颜色
// 必须能分辨(旧版纯哈希取模,实测 7 条里两条撞成同一色)。冲突时取"离已占色号
// 最远"的空位;色号用尽(标注多于 12 条)才允许重复。
function pickTagTone(identity, usedTones) {
  const seed = tagHash(identity) % TAG_TONE_COUNT;
  if (!usedTones.has(seed)) return seed;
  let best = seed;
  let bestGap = -1;
  for (let tone = 0; tone < TAG_TONE_COUNT; tone += 1) {
    if (usedTones.has(tone)) continue;
    let gap = Infinity;
    for (const taken of usedTones) {
      const raw = Math.abs(tone - taken);
      gap = Math.min(gap, Math.min(raw, TAG_TONE_COUNT - raw));
    }
    if (gap > bestGap) {
      bestGap = gap;
      best = tone;
...
// 标注目录:一条标注一条记录(带它的全部词条)。色号分配在此完成,
// 与词条索引分开——命中判定要按"整条标注"来做,不能只看单个词条。
function buildTagCatalog(items) {
  const entries = [];
  for (const item of Array.isArray(items) ? items : []) {
    ...
    entries.push({ item, terms, identity: String(identity) });
  }
  // 稳定顺序:配色只取决于标注集合本身,与接口返回顺序无关(编辑标注会刷新
  // updated_at 而改动接口排序,不排序的话色号会跟着跳)
  entries.sort((a, b) => (a.identity < b.identity ? -1 : a.identity > b.identity ? 1 : 0));
  const usedTones = new Set();
  return entries.map(({ item, terms, identity }) => {
    const tone = pickTagTone(identity, usedTones);
    usedTones.add(tone);
    return {
...
// 词条 → 标注 查表(悬停/复制用 DOM 上的 data-tag-key,这张表主要给
// "全部词条齐备"的判定做底)。同一词条属多条标注时保留先到的一条。
function tagIndexFromCatalog(catalog) {
  const index = new Map();
  for (const tag of Array.isArray(catalog) ? catalog : []) {
...
function buildTagIndex(items) {
  return tagIndexFromCatalog(buildTagCatalog(items));
}
...
function currentTagCatalog() {
  return typeof state !== "undefined" && Array.isArray(state.tagCatalog) ? state.tagCatalog : [];
}

// 命中口径:标注是**整体**——它声明的全部词条都出现在正文里,该标注才生效;
// 生效后它的所有词条统一染色,未生效的标注既不出芯片也不染色任何一个词条。
// 只看"某个词条出现过"会把整条标注显示出来(现场口径:"内容要全才标注")。
function tagIndexForPrompts(prompts) {
  const catalog = currentTagCatalog();
  if (!catalog.length) return currentTagIndex();
  const present = new Set();
  for (const text of promptTextValues(prompts)) {
    for (const segment of String(text).replace(/\r\n/g, "\n").split(/[\n,]+/)) {
      const term = normalizeTagTerm(segment);
      if (term) present.add(term);
    }
  }
  const index = new Map();
  for (const tag of catalog) {
    if (!tag.terms.every((term) => present.has(term))) continue;
    for (const term of tag.terms) {
      if (!index.has(term)) index.set(term, tag);
    }
  }
...
function tagForSegment(value, index = null) {
  const lookup = index instanceof Map ? index : currentTagIndex();
  return lookup.get(normalizeTagTerm(value));
}

function highlightPromptText(value, index = null) {
  return String(value ?? "")
    .replace(/\r\n/g, "\n")
    .split(/(,|\n)/)
    .map((segment) => {
      ...
      const tag = tagForSegment(core, index);
...
function matchedTagsForPrompts(prompts, index = null) {
  const lookup = index instanceof Map ? index : tagIndexForPrompts(prompts);
  const matches = new Map();
  for (const text of promptTextValues(prompts)) {
    for (const segment of String(text).replace(/\r\n/g, "\n").split(/[\n,]+/)) {
      const tag = lookup.get(normalizeTagTerm(segment));
...
// 标注的命中位置落在哪个分支:只在负面 prompt 里出现过的标注,与「负面 prompt
// 默认折叠」联动(现场要求)——负面区块收起时它的芯片一并隐藏,展开后再显示。
// 判定看**出现位置**而不是分类:标注的证据在负面里,就该跟着负面一起藏。
function tagKeysFromNegativeBranchOnly(prompts, index) {
  const onlyNegative = new Set();
  if (!prompts || Array.isArray(prompts) || !(index instanceof Map)) return onlyNegative;
  const positive = new Set();
  const negative = new Set();
  const collect = (entries, bucket) => {
    for (const text of promptTextValues(entries)) {
      for (const segment of String(text).replace(/\r\n/g, "\n").split(/[\n,]+/)) {
        const tag = index.get(normalizeTagTerm(segment));
        if (tag) bucket.add(tag.key);
      }
    }
  };
  collect(prompts.positive || [], positive);
  collect(prompts.negative || [], negative);
  for (const key of negative) {
    if (!positive.has(key)) onlyNegative.add(key);
  }
  return onlyNegative;
}

function tagMatchesHtml(prompts, index = null) {
  const matches = matchedTagsForPrompts(prompts, index);
  if (!matches.length) return "";
  const lookup = index instanceof Map ? index : tagIndexForPrompts(prompts);
  const negativeOnly = tagKeysFromNegativeBranchOnly(prompts, lookup);
  return `<div class="tag-match-panel" aria-label="命中的标记"><span class="tag-match-label">标记</span><div class="tag-match-list">${matches
    .map((tag) => `<button class="tag-match-chip${negativeOnly.has(tag.key) ? " is-negative-sourced" : ""}" type="button" data-tag-key="${escapeHtml(tag.key)}" data-tag-tone="${tag.tone}" data-copy-tag-text="${escapeHtml(tag.name)}" title="${escapeHtml(`${tag.category} · ${tag.name}${tag.note ? ` · ${tag.note}` : ""} · 点击复制`)}">${escapeHtml(tag.name)}</button>`)
    .join("")}</div></div>`;
}
...
const danbooruState = {
  ...
  dockExpanded: false, // 详情侧栏就地面板的展开态(默认折叠只看译文;会话内记忆)
};
...
function openDanbooruPanel(rect, tag, payload, full, img = null, anchor = null) {
  // 面板固定期间不响应其他 tag 的悬停/右键:固定面板是"钉住"的参考,不允许被替换
  if (danbooruState.pinned) return;
  closeDanbooruPanel();
  // 详情弹窗内改「就地面板」:浮层 z-index(30)低于弹窗(1000)会被盖住,而抬到
  // 弹窗之上又会遮住正文/图片 —— 挂在侧栏末尾贴底,内容不重排(见 mountDanbooruPanelDocked)
  const dockHost = anchor instanceof Element ? anchor.closest("#imagePreviewDetails") : null;
  const panel = document.createElement("div");
  panel.className = `danbooru-panel${full ? " danbooru-panel--full" : ""}${dockHost ? " danbooru-panel--dock" : ""}`;
  ...
  if (dockHost) {
    mountDanbooruPanelDocked(panel, dockHost);
    bindDockedPanelToggle(panel);
  } else {
  ...
  if (pinBtn) {
    if (dockHost) {
      // 就地面板不需要"钉住"概念(它已经在正文里,滚动也不关),换成关闭按钮
      pinBtn.classList.add("danbooru-dock-close");
      pinBtn.removeAttribute("data-danbooru-pin");
      pinBtn.removeAttribute("aria-pressed");
      pinBtn.title = "关闭";
      pinBtn.setAttribute("aria-label", "关闭");
      pinBtn.textContent = "×";
      pinBtn.addEventListener("click", () => closeDanbooruPanel());
    } else {
...
// 详情侧栏内的面板:挂在侧栏**末尾**并贴底(sticky)。
// 为什么不插在悬停那一行下面:插入会让它下面的内容整体下移,连续扫过多个 tag 时
// 面板还要挪位置,每次悬停两轮重排——就是现场看到的「跳变」。固定末尾 + sticky:
//   · 追加在末尾 → 上方内容永不重排(它下面已经没有内容了);
//   · sticky bottom → 面板始终停在侧栏可视区底部,不必再 scrollIntoView 去滚它;
//   · 单一槽位 → 换 tag 只换内容,盒子不动。
function mountDanbooruPanelDocked(panel, dockHost) {
  dockHost.appendChild(panel);
}

// 就地面板默认折叠:只留一行译文,点标题行展开/收起。展开态在会话内记忆——
// 连续看多个 tag 时不用每次重新点开。
function setDockedPanelCollapsed(panel, collapsed) {
  panel.classList.toggle("danbooru-panel--collapsed", collapsed);
  danbooruState.dockExpanded = !collapsed;
  const head = panel.querySelector(".danbooru-panel-head");
  if (!head) return;
  head.setAttribute("aria-expanded", String(!collapsed));
  const caret = head.querySelector(".danbooru-dock-caret");
  if (caret) caret.textContent = collapsed ? "▸" : "▾";
}

// 折叠条内容 = 译文;没有译文(资产未启用 / 该词无中文)时用 tag 名兜底,不留空条。
function bindDockedPanelToggle(panel) {
  const head = panel.querySelector(".danbooru-panel-head");
  if (!head) return;
  if (!head.querySelector(".danbooru-panel-tr")) {
    const fallback = document.createElement("span");
    fallback.className = "danbooru-panel-tr";
    fallback.textContent = String(panel.dataset.qaTag || "").replace(/_/g, " ");
    const nameEl = head.querySelector(".danbooru-panel-tag");
    if (nameEl) nameEl.after(fallback);
    else head.prepend(fallback);
  }
  const caret = document.createElement("span");
  caret.className = "danbooru-dock-caret";
  caret.setAttribute("aria-hidden", "true");
  caret.textContent = "▸";
  const pinBtn = head.querySelector(".danbooru-pin");
  if (pinBtn) head.insertBefore(caret, pinBtn);
  else head.appendChild(caret);
  head.setAttribute("role", "button");
  head.setAttribute("tabindex", "0");
  head.setAttribute("aria-label", "展开/收起 danbooru 面板");
  const toggle = () => {
    setDockedPanelCollapsed(panel, !panel.classList.contains("danbooru-panel--collapsed"));
  };
  head.addEventListener("click", (event) => {
    // 头部里的按钮(关闭)自己处理点击,不触发折叠切换
  ...
  setDockedPanelCollapsed(panel, !danbooruState.dockExpanded);
}
...
// HiDPI:缩略图按设备像素比取图。此前固定 w=420/h=420,而卡片里最大显示 250×340
// CSS px——DPR>=2 时物理像素要 500+,固定 420 会被放大发虚。DPR 1 时尺寸与旧值一致
// (420),不改变现有行为;上限 2x 控制体积与解码成本。
function thumbPxFor(cssPx) {
  const dpr = typeof window !== "undefined" && window.devicePixelRatio ? window.devicePixelRatio : 1;
  return Math.round(cssPx * Math.min(2, Math.max(1, dpr)));
}

// 缩略图盒子预留尺寸:懒加载 <img> 未到位时盒子高 0,加载后按图片比例撑开。单列时
// 只是本行变高,双列(CSS columns)会因此**整体重排**——滚动中表现为"右侧列信息位置
// 突然变动"(实测第 9 行起盒子 0 → 312px,联动整列换位)。这里按记录里的原始宽高把
// contain 进 250×340 后的尺寸先写死(与 .hud-thumb-box img 的 max-width/max-height
// 对齐、不放大),图片到位时盒子尺寸不再变化。宽高缺失时返回空串,退回旧行为。
function thumbBoxStyle(width, height) {
  const w = Number(width) || 0;
  const h = Number(height) || 0;
  if (!(w > 0) || !(h > 0)) return "";
  const fit = Math.min(250 / w, 340 / h, 1);
  return ` style="width:${Math.round(w * fit)}px;height:${Math.round(h * fit)}px"`;
}

function favoriteCardHtml(fav, selected = false) {
  const sha = String(fav.sha256 || "");
  const favPx = thumbPxFor(420);
  const thumb = sha
    ? apiUrl(`/api/thumb/${encodeURIComponent(sha)}?w=${favPx}&h=${favPx}`)
    : "";
...
// 收藏卡片参数摘要(批次详情补全后注入):与列表 HUD 卡片同口径取必要信息——
// 基座模型 → 采样器/步数/CFG → 逐图 Seed → 尺寸·张数。收藏接口本身只回书签
// 快照,故这里由 enrichFavoriteCards 按批次补齐后再渲染;补不到的卡片保持原样。
function favoriteCardParamsHtml(item, sha) {
  const images = (item?.batch?.images) || item?.images || [];
  const target = images.find((im) => im && im.sha256 === sha) || images[0] || {};
  const samplers = Array.isArray(item?.samplers) ? item.samplers : [];
  const sampler = samplers[0] || {};
  let baseModel = linkValueText(item?.model?.base_model);
  if (!baseModel || baseModel === "-") {
    const node = (item?.model?.nodes || []).find((n) => n.ckpt_name || n.unet_name || n.model_name);
    baseModel = node ? linkValueText(node.ckpt_name || node.unet_name || node.model_name) : "-";
  }
  // 逐图 seed:batch.seed_images 与 images index 对齐(多图批次每张自己的 seed)
  const index = Math.max(0, images.findIndex((im) => im && im.sha256 === sha));
  const seedImage = (item?.batch?.seed_images || [])[index] || {};
  const seed = (seedImage.seeds || []).find((value) => value !== null && value !== undefined && value !== "");
  const seedText = seed !== undefined ? String(seed) : linkValueText(sampler.seed);
  const parts = [];
  if (sampler.sampler_name) {
    parts.push(`<span>${escapeHtml(sampler.scheduler ? `${sampler.sampler_name} / ${sampler.scheduler}` : String(sampler.sampler_name))}</span>`);
  }
  if (sampler.steps != null) parts.push(`<span><strong class="fav-card-num">${escapeHtml(linkValueText(sampler.steps))} 步</strong></span>`);
  if (sampler.cfg != null) parts.push(`<span>CFG <strong class="fav-card-num">${escapeHtml(formatMetric(linkValueText(sampler.cfg)))}</strong></span>`);
  if (seedText && seedText !== "-") parts.push(`<span>Seed <strong class="fav-card-num">${escapeHtml(seedText)}</strong></span>`);
  const dims = target.width && target.height ? `${target.width}×${target.height}` : "";
  const loraCount = ((item?.loras && item.loras.items) || []).length;
  const metaParts = [];
  if (dims) metaParts.push(dims);
  if (loraCount) metaParts.push(`LoRA ${loraCount}`);
  const batchCount = Number(item?.batch?.count) || images.length;
  if (batchCount > 1) metaParts.push(`同批 ${batchCount} 张`);
  if (!parts.length && (!baseModel || baseModel === "-") && !metaParts.length) return "";
  return `
    <div class="fav-card-params">
      ${baseModel && baseModel !== "-" ? `<div class="fav-card-model" title="${escapeHtml(baseModel)}"><span class="fav-card-label">MODEL</span> <strong>${escapeHtml(baseModel)}</strong></div>` : ""}
      ${parts.length ? `<div class="fav-card-sampler">${parts.join("")}</div>` : ""}
      ${metaParts.length ? `<div class="fav-card-filemeta">${escapeHtml(metaParts.join(" · "))}</div>` : ""}
    </div>`;
}
...
async function loadFavoritesView() {
  ...
  void enrichFavoriteCards(items);
  syncFavoriteStars();
  syncUrlState();
}

// 收藏卡片参数摘要补全:收藏接口只回书签快照(文件名/分类/时间),模型与采样器
// 等记录字段按批次补。有 batch_key 的直接批量取详情;缺 batch_key 的按 sha256
// 反查(并发 3,避免一次性打满本地网关)。补到后按卡片就地插入,不整格重绘——
// 保住滚动位置与正在懒加载的缩略图;补不到的卡片维持原样(收藏可以指向已不在
// 库中的图片,这类卡片只有书签信息)。
const FAVORITE_ENRICH_CONCURRENCY = 3;

async function enrichFavoriteCards(items) {
  const favorites = (items || []).filter((fav) => fav && fav.sha256);
  if (!favorites.length) return;
  const missing = favorites.filter((fav) => !fav.batch_key);
  if (missing.length) {
    let cursor = 0;
    const worker = async () => {
      while (cursor < missing.length) {
        ...
        cursor += 1;
        const key = await resolveFavoriteBatchKey(fav);
        if (key) fav.batch_key = key;
      }
    };
    await Promise.all(
      Array.from({ length: Math.min(FAVORITE_ENRICH_CONCURRENCY, missing.length) }, () => worker()),
    );
  }
  const keys = Array.from(
    new Set(favorites.map((fav) => String(fav.batch_key || "")).filter(Boolean)),
  ).filter((key) => !state.favoriteRecords.has(key));
  // 详情接口单次上限 200 个批次键,超出分片
  for (let start = 0; start < keys.length; start += 200) {
    try {
      const payload = await fetchJson("/api/images/details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ batch_keys: keys.slice(start, start + 200) }),
      });
      (payload?.items || []).forEach((item) => {
        const key = String(item?.batch_key || item?.batch?.key || "");
        if (key) state.favoriteRecords.set(key, item);
      });
    } catch {
      /* 摘要缺失不阻断网格 */
    }
  }
  if (!state.favoritesOnly) return; // 视图已切走,不再动 DOM
  const grid = document.querySelector(".fav-grid");
  if (!grid) return;
  const bySha = new Map(favorites.map((fav) => [String(fav.sha256), fav]));
  grid.querySelectorAll(".fav-card").forEach((card) => {
    const fav = bySha.get(card.dataset.favSha || "");
    const metaBox = card.querySelector(".fav-card-meta");
    if (!fav || !metaBox || metaBox.querySelector(".fav-card-params")) return;
    const item = state.favoriteRecords.get(String(fav.batch_key || ""));
    if (!item) return;
    const html = favoriteCardParamsHtml(item, String(fav.sha256));
    if (!html) return;
    const timeRow = metaBox.querySelector(".fav-card-time");
    if (timeRow) timeRow.insertAdjacentHTML("beforebegin", html);
    else metaBox.insertAdjacentHTML("beforeend", html);
  });
}
...
// 收藏条目反查所在批次:快照里的 batch_key 可能缺失(收藏时没有批次上下文)
// 或过期(重扫/重解析后批键变化),此时详情面板会全空。image-refs 对 sha256
// 精确匹配(rank 0)并回带 batch_key,用它把收藏重新挂回库中记录。
async function resolveFavoriteBatchKey(fav) {
  const sha = String(fav.sha256 || "").trim().toLowerCase();
  if (!sha) return "";
  try {
    const payload = await fetchJson(`/api/image-refs?q=${encodeURIComponent(sha)}&limit=10`);
    const items = Array.isArray(payload?.items) ? payload.items : [];
    const lower = (value) => String(value || "").toLowerCase();
    const hit = items.find((it) => lower(it?.sha256) === sha) || items[0];
    return hit ? String(hit.batch_key || "") : "";
  } catch {
    return "";
  }
}

// 取单个批次的详情文档;失败或查无返回 null(调用方据此继续降级)
async function fetchBatchDetails(batchKey) {
  if (!batchKey) return null;
  try {
    const payload = await fetchJson("/api/images/details", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ batch_keys: [batchKey] }),
    });
...
// 定位到该收藏图在批次内的索引;batch_key 缺失或解析不到时按 sha256 反查当前
// 批次再解析;仍未命中才回退轻量视图(仅图,详情字段全空)。
async function openFavoritePreview(fav) {
  ...
  let item = null;
  if (fav.batch_key) {
    tried.push(String(fav.batch_key));
    item = await fetchBatchDetails(fav.batch_key);
  }
  if (!item) {
    const resolvedKey = await resolveFavoriteBatchKey(fav);
    // 反查键与快照键相同时不重复请求
    if (resolvedKey && !tried.includes(resolvedKey)) {
      item = await fetchBatchDetails(resolvedKey);
      // 命中的新键内存回填:同视图内再次打开不再重复反查
      if (item) fav.batch_key = resolvedKey;
    }
  }
  if (item) {
    const images = item.batch?.images || item.images || [];
...
function bindDanbooruInteractions() {
  ...
  const queryAndOpen = async (el, x, y, full) => {
    ...
    openDanbooruPanel(
      ...
      el, // 详情侧栏内改成就地插入(见 openDanbooruPanel)
    );
  };
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      if (!panel) return;
      // 就地面板是正文的一部分(插在侧栏里):滚动是阅读它,不是躲开它,不关
      if (panel.classList.contains("danbooru-panel--dock")) return;
      ...
      <tbody>
        ${items
          .map((item, itemIdx) => {
            ...
            const batchImages = batch.images || item.images || [];
            const img0 = batchImages[0] || {};
            const firstImage = img0.file || img0 || item.file || {};
            const itemFile = item.file || firstImage;
            ...
            const cardThumbPx = thumbPxFor(420);
            const thumbUrl = firstImage.sha256 ? apiUrl(`/api/thumb/${firstImage.sha256}?w=${cardThumbPx}&h=${cardThumbPx}`) : "";
            ...
            const rowKey = batch.key || item.batch_key || firstImage.sha256 || itemFile.sha256 || "";
            const rowSelected = Boolean(rowKey) && state.selection.has(rowKey);

            // Sampler chain parsing
            const s0 = samplers[0] || {};
            const samplerStr = samplers.map((s) => {
              const name = linkValueText(s.sampler_name);
              const sched = s.scheduler ? ` / ${linkValueText(s.scheduler)}` : "";
              return `${name}${sched}`;
            }).join(" ➔ ") || "-";
            const stepsStr = s0.steps != null ? `${linkValueText(s0.steps)} 步` : "";
            const cfgStr = s0.cfg != null ? `CFG: ${formatMetric(linkValueText(s0.cfg))}` : "";
            const denoiseStr = s0.denoise != null ? `Denoise: ${formatMetric(linkValueText(s0.denoise))}` : "";
            const seedStr = s0.seed != null ? `Seed: ${linkValueText(s0.seed)}` : "";
            const latentStr = `${linkValueText(latent.width)}×${linkValueText(latent.height)}`;

            let baseModelName = linkValueText(item.model?.base_model);
            if (!baseModelName || baseModelName === "-") {
              const ckptNode = (item.model?.nodes || []).find(n => n.ckpt_name || n.unet_name || n.model_name);
              if (ckptNode) {
                baseModelName = linkValueText(ckptNode.ckpt_name || ckptNode.unet_name || ckptNode.model_name);
              }
            }
            if (!baseModelName) baseModelName = "-";

            const hasNeg = Boolean(prompts.negative && prompts.negative.length);
            // 本图的"生效标注"集合:全部词条齐备的标注才进来(芯片与染色共用,
            // 保证"标注要么整体命中、要么完全不显示")
            const activeTags = tagIndexForPrompts(prompts);
            const negCollapsedPref = uiPref("negativePromptCollapsedByDefault", true);

            return `
              <tr data-row-key="${escapeHtml(rowKey)}"${rowSelected ? ' class="is-selected"' : ""}>
                <td colspan="4" class="hud-card-cell">
                  <div class="hud-card${rowSelected ? ' is-selected' : ''}">
                    <!-- 左侧视觉面板: 放大缩略图 (260px) + 完整文件元数据 -->
                    <div class="hud-visual-pane">
                      <div class="hud-visual-top">
                        ${
                          rowKey
                            ? `<input type="checkbox" class="row-check" data-select-row="${escapeHtml(rowKey)}" aria-label="选择本行"${rowSelected ? " checked" : ""} />`
                            : ""
                        }
                        <span class="hud-index-badge">#${itemIdx + 1}</span>
                      </div>
                      <div class="hud-thumb-box"${thumbBoxStyle(thumbUrl ? firstImage.width || itemFile.width : 0, thumbUrl ? firstImage.height || itemFile.height : 0)}>
                        <button
                          class="thumb-link"
                          type="button"
                          ${thumbUrl ? "" : "disabled"}
                        ...
                        >
                          ${thumbUrl ? `<img src="${thumbUrl}" alt="${escapeHtml(displayFilename)}" loading="lazy" />` : '<span class="muted text-xs">无预览</span>'}
                        </button>
                        <span class="lineage-mark" data-lineage-sha="${escapeHtml(itemFile.sha256 || "")}"></span>
                      </div>
                      <div class="hud-visual-bottom">
                        <span class="hud-filename" title="${escapeHtml(displayFilename)}">${escapeHtml(displayFilename)}</span>
                        <div class="hud-meta-sub">${escapeHtml(dimsText + dimsExt)} • ${formatCapturedAt(item.captured_at, item.created_date, item.created_hour)}</div>
                        <div class="hud-path-row" title="${escapeHtml(displayPath)}">${escapeHtml(displayPath)}</div>
                        <div class="hud-batch-summary" title="${escapeHtml(batchSummaryText)}">${escapeHtml(batchSummaryText)}</div>
                        ${
                          batch.files_preview?.length
                            ? `<div class="hud-batch-files" title="${escapeHtml(batch.files_preview.join('\n'))}">${batch.files_preview.map((name) => escapeHtml(name)).join(" · ")}</div>`
                            : ""
                        }
                        <div class="file-dims" style="display:none;">${escapeHtml(dimsText + dimsExt)}</div>
                        <div class="batch-summary" style="display:none;" title="${escapeHtml(batchSummaryText)}">${escapeHtml(batchSummaryText)}</div>
                        ${detailPending ? "" : tagMatchesHtml(prompts, activeTags)}
                      </div>
                    </div>

                    <!-- 右侧主体: 参数状态 Ribbon + 正负提示词双轨画布 (支持折叠) -->
                    <div class="hud-main-pane">
                      <div class="hud-param-ribbon">
                        <!-- 第一行: 基模与加载器、Latent 尺寸与衍生摘要 -->
                        <div class="hud-ribbon-row">
                          <span class="hud-badge hud-badge-model">MODEL</span>
                          <span class="font-bold text-[12px]">${escapeHtml(baseModelName)}</span>
                          ${
                            (item.model?.nodes || [])
                              .filter(n => n.node_type === 'VAELoader' || n.node_type === 'CLIPLoader')
                              .map(n => `<span class="hud-badge hud-badge-loader">${n.node_type === 'VAELoader' ? 'VAE' : 'CLIP'}: ${escapeHtml(linkValueText(n.vae_name || n.clip_name))}</span>`)
                              .join("")
                          }
                          <span class="hud-badge hud-badge-latent">Latent: ${escapeHtml(latentStr)} (batch ${latent.batch_size || 1})</span>
                          <span class="derived-summary" data-derived-sha="${escapeHtml(firstImage.sha256 || "")}"></span>
                        </div>

                        <!-- 第二行: LoRA 全量可交互芯片 (含强度及过滤) -->
                        ${
                          loras.length > 0 ? `
                            <div class="hud-ribbon-row hud-lora-row">
                              <span class="hud-badge hud-badge-lora">LoRA (${loras.length})</span>
                              <div class="hud-chip-list">
                                ${loras.map((lora) => loraChipHtml(lora)).join("")}
                              </div>
                            </div>
                          ` : ''
                        }

                        <!-- 第三行: 采样器全息链条 + 步数/CFG/Denoise/Seed 关键指标 -->
                        <div class="hud-ribbon-row text-[11px] text-[var(--muted)]">
                          <span>采样: <strong class="hud-param-val">${escapeHtml(samplerStr)}</strong></span>
                          ${stepsStr ? `<span class="hud-sep">•</span><span><strong class="hud-param-val hud-num-val">${escapeHtml(stepsStr)}</strong></span>` : ''}
                          ${cfgStr ? `<span class="hud-sep">•</span><span><strong class="hud-param-val hud-num-val">${escapeHtml(cfgStr)}</strong></span>` : ''}
                          ${denoiseStr ? `<span class="hud-sep">•</span><span><strong class="hud-param-val hud-num-val">${escapeHtml(denoiseStr)}</strong></span>` : ''}
                          ${seedStr ? `<span class="hud-sep">•</span><span class="hud-seed-val">${escapeHtml(seedStr)}</span>` : ''}
                          ${
                            samplerCount > 1
                              ? `<span class="hud-sep">•</span><span>(${samplerCount} 阶段采样)</span>`
                              : ""
                          }
                        </div>
                        ${
                          samplerCount > 1 ? `
                            <div class="hud-ribbon-row hud-sampler-stages text-[11px]">
                              ${samplers.map((s, sIdx) => {
                                const sName = linkValueText(s.sampler_name);
                                const sSched = s.scheduler ? ` / ${linkValueText(s.scheduler)}` : "";
                                const sSteps = s.steps != null ? `${linkValueText(s.steps)}步` : "";
                                const sCfg = s.cfg != null ? `CFG ${formatMetric(linkValueText(s.cfg))}` : "";
                                const sDen = s.denoise != null ? `Denoise ${formatMetric(linkValueText(s.denoise))}` : "";
                                const sSeed = s.seed != null ? `Seed ${linkValueText(s.seed)}` : "";
                                return `<span class="hud-stage-pill"><strong class="hud-stage-num">阶段 ${sIdx + 1}:</strong> ${escapeHtml(sName)}${escapeHtml(sSched)} · ${escapeHtml(sSteps)} · ${escapeHtml(sCfg)} · ${escapeHtml(sDen)}${sSeed ? ` · <span class="hud-seed-val">${escapeHtml(sSeed)}</span>` : ""}</span>`;
                      ...
                      <!-- 提示词画布: 双轨自适应分布，彻底移除 [POS]/[NEG] 人造占位标签，保留完整折叠与复制控件 -->
                      <div class="hud-prompt-canvas">
                        <div class="hud-positive-box prompt-cell prompt-cell--positive">
                          ${detailPending ? '<div class="muted">正在加载详细信息...</div>' : `${manualLabelMatchesHtml(manualLabelMatches)}${promptSummary(prompts.positive, rawPrompt, { tagIndex: activeTags, branch: "positive" })}`}
                        </div>
                        ${
                          hasNeg ? `
                            <div class="hud-negative-box prompt-cell prompt-cell--negative">
                              ${detailPending ? '<div class="muted">正在加载详细信息...</div>' : promptSummary(prompts.negative, rawPrompt, { tagIndex: activeTags, branch: "negative", ...(negCollapsedPref ? { entryCollapsed: true } : {}) })}
                            </div>
                          ` : ''
                        }
                      </div>
    ...
    for (const cn of entry.controlnets || []) {
      ...
      chips.push(
        `<span class="chip cn-chip${cn.bypassed ? " cn-chip-bypassed" : ""}" title="${escapeHtml(title)}">` +
          // 显示完整模型名(此前截到 18 字符,现场反馈"显示的不是全名");行内可换行
          `CN ${escapeHtml(cn.name || cn.node_type || "?")}` +
          (cn.bypassed ? " (灰)" : "") +
          `</span>`
      );
    }
...
function renderPreviewDetails() {
  ...
  const samplers = item.samplers || [];
  const prompts = item.prompts || { positive: [], negative: [] };
  // 与列表卡片同源:全部词条齐备的标注才生效(芯片 + 染色共用同一个集合)
  const activeTags = tagIndexForPrompts(prompts);
  const matchedTagHtml = tagMatchesHtml(prompts, activeTags);
  ...
  const novelaiHtml = rawNovelai
    ? `
    <section class="detail-section detail-enrichment prompt-entry is-collapsed">
      <div class="detail-enrichment-head prompt-entry-header">
        <h3>来源</h3>
        <div class="prompt-entry-actions">
          <span class="detail-enrichment-status">NovelAI</span>
          <button class="prompt-entry-toggle" type="button" data-toggle-entry aria-label="展开/折叠来源详情" title="展开/折叠来源详情">+</button>
        </div>
      </div>
      <div class="prompt-entry-body">${novelaiRawJsonBlock("NovelAI 原始元数据", rawNovelai)}</div>
    </section>`
    : "";
  ...
  container.innerHTML = `
    ...
    <section class="detail-section">
      <h3>正向 Prompt</h3>
      ${promptSummary(displayPositivePrompts, null, { tagIndex: activeTags, branch: "positive" })}
    </section>
    <section class="detail-section">
      <h3>负向 Prompt</h3>
      ${promptSummary(displayNegativePrompts, null, { tagIndex: activeTags, branch: "negative" })}
    </section>
...
function renderLineageSection(activeSha) {
  ...
  // 有已匹配血缘默认展开,无则收起;两种情况都给折叠控件(复用 negative 区块机制)。
  // 收起态由 head 徽标承载信息量:计数,或「源图未入库」。
  const hasMatched = parents.length > 0 || children.length > 0;
  const statusChip = hasMatched
    ? [parents.length ? `父图 ${parents.length}` : "", children.length ? `子图 ${children.length}` : ""].filter(Boolean).join(" · ")
    : unresolved.length
      ? "源图未入库"
      : "";
  return `<section class="detail-section detail-lineage prompt-entry${hasMatched ? "" : " is-collapsed"}">
    <div class="prompt-entry-header">
      <h3>继承关系</h3>
      <div class="prompt-entry-actions">
        ${statusChip ? `<span class="detail-enrichment-status">${statusChip}</span>` : ""}
        <button class="prompt-entry-toggle" type="button" data-toggle-entry aria-label="展开/折叠继承关系" title="展开/折叠继承关系">${hasMatched ? "−" : "+"}</button>
      </div>
    </div>
...
async function copyPromptLayer(trigger) {
  const container = trigger.closest(".prompt-layer") || trigger.closest(".prompt-entry");
  const content = container?.querySelector(".prompt-pre")?.textContent || "";
  if (!content.trim()) {
    return;
  }
...
// 「已生效筛选」统一渲染在右下角浮动 chip 栈里(与 LoRA/收藏分类栈同处):
// 覆盖模型/文件名/关键词/排除词/起止时间/只看收藏,点 chip 即移除该项并重新查询。
// LoRA 与收藏分类各自 combo 自带 chip 栈(createMultiLoraCombo),此处不重复。
const ACTIVE_FILTER_DEFS = [
  {
    key: "keyword",
    label: "关键词",
    read: () => document.getElementById("searchInput").value.trim(),
    clear: () => { document.getElementById("searchInput").value = ""; },
  },
  {
    key: "filename",
    label: "文件名",
    read: () => document.getElementById("filenameInput").value.trim(),
    clear: () => { document.getElementById("filenameInput").value = ""; },
  },
  {
    key: "base_model",
    label: "模型",
    read: () => document.getElementById("modelComboboxInput").value.trim(),
    clear: () => {
      const clearBtn = document.getElementById("modelClearBtn");
  ...
  },
  {
    key: "exclude_q",
    label: "排除词",
    read: () => document.getElementById("excludeKeywordsInput").value.trim(),
    clear: () => { document.getElementById("excludeKeywordsInput").value = ""; },
  },
  {
    key: "from_date",
    label: "起",
    read: () => document.getElementById("fromDate").value.trim(),
    clear: () => { document.getElementById("fromDate").value = ""; },
  },
  {
    key: "to_date",
    label: "止",
    read: () => document.getElementById("toDate").value.trim(),
    clear: () => { document.getElementById("toDate").value = ""; },
  },
  {
    key: "fav",
    label: "只看收藏",
    read: () => (document.getElementById("favoriteOnlyInput").checked ? "1" : ""),
    clear: () => { document.getElementById("favoriteOnlyInput").checked = false; },
  },
];

function syncActiveFilterStacks() {
  const stack = document.getElementById("activeFilterStack");
  const chips = document.getElementById("activeFilterChips");
  if (!stack || !chips) return;
  const items = [];
  for (const def of ACTIVE_FILTER_DEFS) {
    const value = def.read();
    if (value) items.push({ key: def.key, label: def.label, text: value === "1" ? def.label : `${def.label}: ${value}` });
  }
  chips.innerHTML = items
    .map(
      (item) =>
        `<button class="chip lora-chip lora-stack-chip" type="button" data-clear-active-filter="${escapeHtml(item.key)}" title="点击移除 ${escapeHtml(item.text)}">` +
        `<span>${escapeHtml(item.text)}</span></button>`,
    )
    .join("");
  stack.hidden = items.length === 0;
  // 计数取全量筛选(currentQuery),不是本栈条数——LoRA/收藏分类在各自的栈里
  syncFiltersFoldBadge();
}

function clearActiveFilter(key) {
  const def = ACTIVE_FILTER_DEFS.find((item) => item.key === key);
  if (!def) return;
  def.clear();
  applyFilters().catch((error) => showToast(error.message, { type: "error" }));
}

function bindActiveFilterStacks() {
  const chips = document.getElementById("activeFilterChips");
  if (!chips) return;
  chips.addEventListener("click", (event) => {
    const trigger = event.target.closest("[data-clear-active-filter]");
    if (!trigger) return;
    clearActiveFilter(trigger.dataset.clearActiveFilter);
  });
}

// 折叠浮标只报数(具体条件在右下角 chip 栈里,可点击移除,避免两处重复展示)
function activeFilterCount() {
  const params = new URLSearchParams(currentQuery());
  let count = 0;
  params.forEach((value, key) => {
    if (key === "group_mode" || key === "page" || key === "limit" || key.endsWith("_mode")) return;
    if (key === "lora" || key === "exclude_lora") {
      count += 1;
      return;
    }
    if (key === "fav_cat") {
...
function syncFiltersFoldBadge(count = null) {
  const box = document.getElementById("filtersFoldChips");
  if (!box) return;
  const n = count === null ? activeFilterCount() : count;
  box.innerHTML = n
    ? `<span class="filters-fold-chip">${n} 项筛选在生效</span>`
    : "";
}

// 悬浮筛选面板不占文档流,列表需要按面板实际高度预留顶部空带,否则首卡会被面板盖住。
// 预留量在折叠/展开间保持不变——若随折叠收回,列表会在折叠瞬间整体上跳。
function syncFiltersReserve() {
  const panel = document.querySelector(".filters");
  const workspace = document.querySelector(".split-workspace");
  if (!panel || !workspace) return;
  // 折叠态收回预留:内容整体上移,填掉面板让出的空档(用户明确要求)
  if (panel.classList.contains("is-folded")) {
    document.documentElement.style.setProperty("--filters-reserve", "0px");
    return;
  }
  const prev = parseFloat(getComputedStyle(workspace).marginTop) || 0;
  const panelRect = panel.getBoundingClientRect();          // fixed:视口坐标,与滚动无关
  const naturalTop = workspace.getBoundingClientRect().top + window.scrollY - prev;
  // 只需补齐"面板底部"与"内容自然起点"的差值:直接用面板高度会多留一条空档
  // (面板顶部在内容自然起点之上,那一段不该重复预留)
  const need = Math.max(0, Math.round(panelRect.bottom - naturalTop));
  document.documentElement.style.setProperty("--filters-reserve", `${need}px`);
  // 面板底边(视口坐标;fixed 定位,与滚动无关):右上角「状态摘要」弹层据此让位
  document.documentElement.style.setProperty("--filters-bottom", `${Math.round(panelRect.bottom)}px`);
}

function bindFilterFold() {
  const panel = document.querySelector(".filters");
  const handle = document.getElementById("filtersFoldHandle");
  const foldBtn = document.getElementById("filtersFoldBtn");
  if (!panel) return;
  let lastY = window.scrollY;
  // 折叠会改变内容高度,浏览器随之修正滚动位置并派发 scroll 事件(看起来像"向上滚"),
  // 不屏蔽就会出现"刚折叠就被自己展开"的反复。窗口期内的 scroll 只同步基准,不判定。
  let ignoreScrollUntil = 0;
  const setFolded = (folded) => {
    if (panel.classList.contains("is-folded") === folded) return;
    syncFiltersFoldBadge();
    panel.classList.toggle("is-folded", folded);
    // 预留带跟随折叠:折叠→收回(内容上移填满空档);展开→补回,并补偿滚动
    // 抵消这次高度变化,避免视口里的内容整体跳一下
    syncFiltersReserve();
    // 位移交给 .split-workspace 的 margin-top 过渡(180ms):折叠=内容上移填充,
    // 展开=内容下移让位。这里不做 scrollBy 补偿——它与 CSS 过渡叠加会变成
    // "内容在动、视口也在动"的双重位移。
    ignoreScrollUntil = performance.now() + 320;
    if (handle) {
      // 用类而非 hidden:display 切换会让过渡失效,浮标就"跳"出来了
      handle.classList.toggle("is-shown", folded);
      handle.setAttribute("aria-hidden", folded ? "false" : "true");
      handle.tabIndex = folded ? 0 : -1;
    }
  };
  window.addEventListener("scroll", () => {
    ...
    // 用位移量而非方向比较:同位置的尾随事件(y === lastY)或 1px 级抖动不得翻转状态
    const dy = y - lastY;
    lastY = y;
    // 滚动只负责"向下折叠";**展开一律手动**(点浮标)——向上滚、回到顶部都不再自动展开
    if (panel.contains(document.activeElement)) return;   // 正在填筛选时不折走面板
    if (dy > 8 && y > 80) setFolded(true);
    // |dy| <= 8 或向上滚:保持现状
  }, { passive: true });
  if (foldBtn) {
    foldBtn.addEventListener("click", () => {
      ...
      setFolded(true);
      // 焦点留在面板内会让"聚焦不折叠"的守卫失效,收起后把焦点交给浮标
      if (handle) handle.focus();
    });
  }
  if (handle) {
    handle.addEventListener("click", () => {
      // 唯一的展开入口(手动触发):就地展开,不跳转滚动位置
      lastY = window.scrollY;
      setFolded(false);
    });
  }
  // 面板会在两次 resize 之间"长高"——扫描提示条整行、LoRA/收藏夹选中 chip 折行、
  // 命名视图与字体/缩放都会改它的高度,而预留带原先只在 boot/resize/折叠时重算,
  // 实测提示条 +86px、chip +26px 都直接压住首行卡片。盯住面板自身尺寸即可:
  // 面板是 fixed 悬浮层,其高度不依赖工作区,重算不会自激(写入的是工作区 margin,
  // 不改变面板尺寸)。
  if (typeof ResizeObserver === "function") {
    new ResizeObserver(() => syncFiltersReserve()).observe(panel);
  }
}
...
function syncUrlState() {
  ...
  // 每次列表加载后刷新右下角「已选筛选」栈与折叠浮标计数
  syncActiveFilterStacks();
}
...
async function boot() {
  ...
  syncBulkUi();
  bindFilterFold();
  bindActiveFilterStacks();
  syncActiveFilterStacks();
  syncFiltersReserve();
  window.addEventListener("resize", syncFiltersReserve, { passive: true });
